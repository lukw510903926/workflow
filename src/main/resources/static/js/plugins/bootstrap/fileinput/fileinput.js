...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _listenModalEvent: function (event) {
            ...
            $modal.on(event + '.bs.modal', function (e) {
                ...
                if (event === 'shown') {
                    $btnBord.removeClass('active').attr('aria-pressed', 'false');
                    $btnFull.removeClass('active').attr('aria-pressed', 'false');
                    if ($modal2.hasClass('file-zoom-fullscreen')) {
                        self._maximizeZoomDialog();
                        if (checkFullScreen()) {
                            $btnFull.addClass('active').attr('aria-pressed', 'true');
                        } else {
        ...
        _setExtraData: function (uploadExtraData) {
            var self = this;
            self.uploadExtraData = uploadExtraData;
        },
        ...
        refresh4upload: function (options) {
            var self = this, $el = self.$element;
            self.options = options ? $.extend(true, {}, self.options, options) : self.options;
            self._setExtraData(self.options.uploadExtraData);
            self.upload();
            return $el;
        }
    };
